...
const FileInput = ({ onChange }: { onChange: (event: ChangeEvent<HTMLInputElement>) => void }) => (
    <>
        <input
            type="file"
            accept=".pdf,.doc,.docx"
...
const FilePreview = ({ file }: { file: File }) => (
    <p className="text-[#7B7B7B] p-2 underline flex items-center">
        {file.name} <FaCheckCircle className="ml-2 text-green-500" />
    </p>
);

export default function PlatformFeatureUpdates() {
    ...
    const handleCalendarToggle = () => {
        setState((prevState) => ({ ...prevState, showCalendar: !prevState.showCalendar }));
    };

    const handleToastToggle = () => {
        setState((prevState) => ({ ...prevState, showToast: !prevState.showToast }));
    };
    ...
    const handleSend = () => {
        setState((prevState) => {
            if (prevState.selectedFile) {
                return {
                    ...prevState,
                    textAreaValue: prevState.textAreaValue + "\n" + prevState.selectedFile.name,
                    selectedFile: null,
                };
            }
            return prevState;
    ...
    return (
        <div className="flex items-center justify-center bg-[#0A0908] bg-opacity-50 h-screen z-10">
            <div className="bg-[#101010] rounded-[20px] w-1/3">
                <p className="p-5 border-b border-[#181818]">Platform/Feature Updates</p>
                <div className="bg-[#131313] flex flex-col items-center px-8">
                    ...
                    <select className="bg-[#2A2A2A] p-4 rounded-[16px] w-full">
                        <option>Today</option>
                        <option>Tomorrow</option>
                        <option>Next Week</option>
                    </select>
                    <button className="underline w-full text-right text-[#BDFE1C] text-[12px] pt-2 pb-10" onClick={handleCalendarToggle}>Custom day</button>
                    {state.showCalendar && (
                        <CalendarWidget />
                    )}
                </div>
                <div className="bg-[#1B1B1B] px-10 rounded-b-[20px]">
                    ...
                    <p className="flex flex-col text-[14px] pb-3 ">Describe the upcoming platform or feature updates Include the:
                        <li className="text-[#535353] pl-3">Name of the feature</li>
                        <li className="text-[#535353] pl-3">Details of the update.</li>
                        <li className="text-[#535353] pl-3">Expected release date.</li>
                        <li className="text-[#535353] pl-3"> How it improves the platform.</li>
                    </p>
                </div>
                <div className="bg-[#131313] pt-10 px-4 border-b border-[#131313]">
                    <textarea
                        ...
                        onChange={(e) => setState((prevState) => ({ ...prevState, textAreaValue: e.target.value }))}
                    />
                    ...
                    {state.selectedFile && <FilePreview file={state.selectedFile} />}
                </div>
                <div className="bg-[#131313] flex justify-center items-center py-5 rounded-[20px]">
                    <button className="bg-gradient-to-r from-[#03FFA3] to-[#7F56D9] px-14 py-3 rounded-[20px]" type="submit" onClick={handleToastToggle}>
                        Save
                    </button>
                </div>
            </div>
